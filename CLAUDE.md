...
`DP2Config.template_mode` (default `'multiband'`) decides which template physics is used. `'multiband'` rebuilds the loaded template with `dust=None, betas=None`, so `TemplateFitter` takes its Multiband path: free `mu_<band>` per band, shared `A` and `phi`. `'rr'` keeps the rr-templates path (`mu`, `EBV`, and the PLR term `beta_b(P)`). The `'rr'` path is NOT the default because `beta_b` is period-dependent: a star off the assumed RRab colour/luminosity locus is penalised increasingly with period, so RSS slopes upward across the grid (+23543/day measured on objectId 738172043433898052) and the minimum is set by that slope rather than by phase information, pinning the fit near `pmin`. Multiband on the same object: slope -30/day, period 0.69681 vs PeriodSearch 0.696535, chi2/dof 23.1 -> 3.2. Across the 17 known DP2 RRL: 11/17 vs 5/17 agreement with PeriodSearch, median chi2/dof 8.1 vs 94.6, 0/17 vs 4/17 railed. Note this changes the output schema (`mu_g, mu_r, ... , A` instead of `mu, EBV, A`) — `fit_meta` handles both. `template_mode` is validated before the template file is read. `des_correction` is skipped in multiband mode (it shifts `betas`).
